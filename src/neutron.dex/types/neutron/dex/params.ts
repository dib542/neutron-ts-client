...
function createBaseParams(): Params {
  return { feeTiers: [], maxTrueTakerSpread: "" };
}
...
export const Params = {
        ...
        case 1:
          if ((tag & 7) === 2) {
            ...
            while (reader.pos < end2) {
              message.feeTiers.push(longToNumber(reader.uint64() as Long));
            }
          } else {
            message.feeTiers.push(longToNumber(reader.uint64() as Long));
          }
  ...
  fromJSON(object: any): Params {
    return {
      feeTiers: Array.isArray(object?.feeTiers) ? object.feeTiers.map((e: any) => Number(e)) : [],
  ...
  toJSON(message: Params): unknown {
    const obj: any = {};
    if (message.feeTiers) {
      obj.feeTiers = message.feeTiers.map((e) => Math.round(e));
    } else {
      obj.feeTiers = [];
    }
  ...
  fromPartial<I extends Exact<DeepPartial<Params>, I>>(object: I): Params {
    const message = createBaseParams();
    message.feeTiers = object.feeTiers?.map((e) => e) || [];
